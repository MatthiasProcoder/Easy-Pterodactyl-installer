...
show_banner() {
    ...
    echo ""
    echo -e "${YELLOW}📋 Features: Panel | Wings | Blueprint | Nebula | Tools Suite${NC}"
...
EOF
    ...
    run_command "mysql -e \"CREATE USER IF NOT EXISTS 'pterodactyl'@'localhost' IDENTIFIED BY '${DB_PASSWORD}';\"" "Creating database user"
    ...
    run_command "cd /var/www/pterodactyl" "Entering panel directory"
    ...
    run_command "sed -i \"s/DB_PASSWORD=.*/DB_PASSWORD=${DB_PASSWORD}/\" .env" "Setting database password"
    ...
    php artisan tinker << EOF >> $INSTALL_LOG 2>&1
\$user = new \App\Models\User();
\$user->email = '${EMAIL}';
\$user->username = 'admin';
\$user->name_first = 'Matthias';
...
EOF
    ...
    echo -e "${BLUE}Step 11: Setting Up Queue Worker...${NC}"
...
EOF
    ...
    echo -e "${BLUE}Step 13: Obtaining SSL Certificate...${NC}"
    ...
    if command -v ufw &> /dev/null; then
        ...
        run_command "echo 'y' | ufw enable" "Enabling firewall"
    fi
    
    echo -e "${BLUE}Step 15: Setting Up Cron Jobs...${NC}"
...
if curl -s -o /dev/null -w "%{http_code}" https://${FQDN:-localhost} | grep -q "200\|301\|302"; then
...
echo ""
echo "6. Last 10 Errors:"
tail -n 10 /var/www/pterodactyl/storage/logs/laravel.log 2>/dev/null | grep -i error | tail -5
...
Wings Version: $(/usr/local/bin/wings --version 2>/dev/null || echo "Latest")
Wings API Port: 8080
...
✅ NEXT STEPS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. In your panel, go to Nodes > Your Node > Configuration
2. Copy the configuration token
3. Paste it in /etc/pterodactyl/config.yml
...
install_nebula() {
    ...
    echo -e "${CYAN}Features: Advanced resource monitoring, Better UI, Performance optimizations${NC}"
    echo ""
...
EOF
    ...
    echo -e "${CYAN}Commands:${NC}"
    echo "  • matthias-backup - Create backup"
    echo "  • matthias-restore - Restore from backup"
    echo ""
...
echo ""
echo "Panel Statistics:"
echo "-----------------"
if [ -d "/var/www/pterodactyl" ]; then
    cd /var/www/pterodactyl
    php artisan p:list:servers 2>/dev/null | head -5
fi
...
check_updates() {
    ...
    if [ -d "/var/www/pterodactyl" ]; then
        ...
        if [ $LOCAL = $REMOTE ]; then
            echo -e "${GREEN}Panel is up to date!${NC}"
        else
            echo -e "${YELLOW}Panel updates available!${NC}"
        fi
    fi
    echo -e "${GREEN}Press Enter to continue...${NC}"
    read
...
main_menu() {
    while true; do
        ...
        echo -e "${WHITE}║${GREEN} 5) ${CYAN}Install Tools Suite${WHITE}                             ║${NC}"
        echo -e "${WHITE}║${GREEN} 6) ${CYAN}Update & Maintenance${WHITE}                           ║${NC}"
        echo -e "${WHITE}║${GREEN} 7) ${CYAN}Uninstall Everything${WHITE}                           ║${NC}"
        echo -e "${WHITE}║${GREEN} 8) ${CYAN}Exit${WHITE}                                           ║${NC}"
        echo -e "${WHITE}╚════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "${CYAN}💬 Need help? Join Discord: https://discord.gg/J2SqrH9nwZ${NC}"
        echo ""
        read -p "$(echo -e ${BOLD_YELLOW}Select an option [1-8]: ${NC})" choice
